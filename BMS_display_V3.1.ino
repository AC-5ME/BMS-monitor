...
void Print_Voltage(long& packVolts, int& packPercent) {      //Reset screen and print pack voltage
  const int packMin = 84.0;    //Pack voltage % conversion (3V to 4V2 per cell)
  const int packMax = 117.6;
...
void Send_Alarm(long& packVolts, long& packDev, int& TH1, int& TH2, int& TH3, int& TH4) {
  ...
  while (((packVolts / 100.0) > 117.60)) {      //pack over voltage
    LoRa.beginPacket();
    LoRa.print ("!");
    LoRa.endPacket();
    analogWrite(alarmPin, 255);
...
void loop() {
  long packVolts = 0;     //Display 0's to indicate failure
  int packPercent = 0;
  long packMean = 0;
  long packDev = 0;
  int TH1 = 0;
